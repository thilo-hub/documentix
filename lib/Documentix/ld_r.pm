...
sub mk_query {
    ...
    my $source =
	($qidx ?
	        qq{select idx,Snippet snippet,qidx,mtime,-rank ord from cache_lst natural cross join cache_q natural join mtime }
		: qq{select idx,Content snippet,mtime,mtime ord from mtime natural cross join m_content }
	)
	...
	;
     ...
     $source =~ s/\sfrom\s/ from (select * from dates where  date between :fromdate and :enddate group by idx) natural join /
     		if $date_from;
     my @add = ();
