...
$dh->do(q{ create table if not exists cache_q ( qidx integer, idx integer, snippet text, unique(qidx,idx))});



my $s1=q{select qidx from cache_lst where query = ?};
my $s2=q{insert or abort into cache_lst (query) values(?)};
my $s3=q{insert into cache_q ( qidx,idx,snippet ) select ?, docid idx,snippet(text) snippet from text where text match ? };
...
if ( $class )
{
	...
	my $sth=$dh->prepare(q{insert into cls (tagid) select tagid from tagname where tagname=?});
...
if ( $search )
{
	...
	my $rest = qq{create table resl as select * from $subsel cache_q where qidx = ?};
	$dh->do($rest,undef,$idx);
	# get list of classes
	$classes= q{select tagname,count(*) from tags natural join tagname where idx in ( select idx from resl) group by 1 order by 1}; 
...
else
{
	...
	$stm1=qq{ select idx,value snippet from $subsel metadata where tag="Content" limit ?,?  };
}
...
print $q->div({-id=>"nresults"},$ndata),
       $q->span({-style=>"display:none"},$q->div({-id=>"classes"},join("",@$classes)));
